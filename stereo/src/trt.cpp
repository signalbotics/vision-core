...
int stereoTRT::Initialize(std::string model_path,int gpu_id,CalibrationParam&Calibrationparam)
{
    INPUT_H = 480;
    INPUT_W = 640;
    ...
    std::string out_engine=model_path;

    bool enginemodel=file_exists(out_engine);
    if (!enginemodel)
    {
        std::cout << "optimizing model.." << std::endl;
        std::string command = "wget -O /tmp/light_480x640.onnx http://packages.signalbotics.com/models/light_480x640.onnx";
        system(command.c_str());
        model_path="/tmp/light_480x640.onnx";
        bool onnx_model=file_exists(model_path);
        if (!onnx_model)
        {
           std::cout<<"model doesn't exist! please make sure there is Internet connection."<<std::endl;
           return -1;
        }
        Onnx2Ttr onnx2trt;
    ...
    CUDA_CHECK(cudaMalloc((void**)&buffers[outputIndex], STEREO_BATCH_SIZE * OUTPUT_SIZE * sizeof(float)));
    ...
    //disparity   
    flow_up=new float[STEREO_BATCH_SIZE * OUTPUT_SIZE];
    return 0;
}

int stereoTRT::runStereo(cv::Mat&rectifyImageL2,cv::Mat&rectifyImageR2,float*pointcloud,cv::Mat&DisparityMap)
{
    ...
    stereo_preprocess(img_left_device, buffers[inputIndex1], INPUT_W, INPUT_H, stream);
    stereo_preprocess(img_right_device, buffers[inputIndex2], INPUT_W, INPUT_H, stream);

    // Run inference
    //(*context).enqueue(STEREO_BATCH_SIZE, (void**)buffers, stream, nullptr);
    (*context).enqueueV2((void**)buffers, stream, nullptr);
    cudaStreamSynchronize(stream);
    stereo_reprojectImageTo3D(img_left_device,buffers[2],PointCloud_devide,Calibrationparam_Q,INPUT_H,INPUT_W);
    

    CUDA_CHECK(cudaMemcpy(flow_up, buffers[2], STEREO_BATCH_SIZE * OUTPUT_SIZE * sizeof(float), cudaMemcpyDeviceToHost));
